...
import { resolveUS, getUSQuote, getUSBatch, getUSCalendar, searchSymbols, getFinnhubFinancials } from "./finnhub";
...
import { twelveDataConfigured, getTwelveDataCandles, getForexQuote, getDividends as getTwelveDataDividends } from "./twelveData";
...
// ---------------------------------------------------------------- financials
// Yahoo (KR — the only free option, since KIS has no fundamentals and
// Twelve Data's free plan doesn't cover Korean symbols) uses a different
// shape than Finnhub's reported financials (US); normalized here into one
// common format so the UI never needs to know which source answered.
...
export async function getFinancials(symbol, market) {
  ...
  // Twelve Data's income_statement/balance_sheet/cash_flow are free-tier
  // accessible for AAPL only (confirmed live — every other symbol 403s as
  // paid-tier) — effectively a demo exception, not a usable free source.
  // Finnhub's /stock/financials-reported (raw SEC XBRL) works broadly.
  try {
    return await getFinnhubFinancials(symbol);
  } catch (e) {
