...
// FHKST03010100 = 국내주식 기간별 시세 (daily/weekly/monthly candles).
// period: "D" | "W" | "M" — reused for every range, not just the default
// quote's ~150-day view (see lib/providers/index.js's getChart).
export async function getKRCandles(code, { period = "D", days = 150 } = {}) {
  const end = new Date();
...
// FHKST03010200 = 주식당일분봉조회 (intraday minute candles). Best-effort,
// single call — KIS returns a recent window ending at FID_INPUT_HOUR_1
// (not a paginated full day), which is enough for a 1D/5D chart view but
// not a complete trading day from the open.
export async function getKRIntraday(code) {
  ...
  return (chart.output2 || [])
    .filter((r) => n(r.stck_prpr))
    .map((r) => ({
      d: `${r.stck_bsop_date.slice(0, 4)}-${r.stck_bsop_date.slice(4, 6)}-${r.stck_bsop_date.slice(6, 8)}T${r.stck_cntg_hour.slice(0, 2)}:${r.stck_cntg_hour.slice(2, 4)}:${r.stck_cntg_hour.slice(4, 6)}`,
      c: n(r.stck_prpr),
    }))
    .reverse();
}
...
export async function getKRQuote(code) {
  ...
  // KIS enforces a tight per-second rate limit; firing the chart call
  // immediately after the price call above can trip it (EGW00201, "초당
  // 거래건수를 초과하였습니다") even for a single user's single request. A
  // short stagger avoids that without meaningfully slowing the response.
  await new Promise((r) => setTimeout(r, 350));

  let closes = [];
  try {
    closes = await getKRCandles(code, { period: "D", days: 150 });
  } catch (e) {
