...
// ------------------------------------------------------------ US: market overview
// Feeds the Market tab's US side. There's no free "whole US market" index
// quote on Finnhub's tier (^GSPC/^DJI/^IXIC 403 as "CFD indices"), so this
// uses the standard ETF proxies instead — SPY/QQQ/DIA track the S&P 500/
// Nasdaq/Dow closely enough for an overview dashboard and are ordinary
// equities on the free tier (confirmed live).
export const US_INDEX_PROXIES = { SPY: "S&P 500", QQQ: "NASDAQ", DIA: "DOW JONES" };

export async function getUSIndices() {
  return getUSBatch(Object.keys(US_INDEX_PROXIES));
}
