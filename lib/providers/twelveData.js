...
// Dividend history — doesn't change intraday, cache generously.
const divCache = new Map();
const DIV_TTL = 24 * 60 * 60 * 1000;

export async function getDividends(symbol) {
  const hit = divCache.get(symbol);
  if (hit && Date.now() - hit.t < DIV_TTL) return hit.d;
  ...
  const res = await fetch(url);
  const data = await res.json();
  if (data.status === "error") throw new Error(`Twelve Data dividends error: ${data.message || "unknown"}`);

  const dividends = (data.dividends || [])
    .map((d) => ({ date: d.ex_date, amount: num(d.amount) }))
    .filter((d) => d.amount !== null);

  divCache.set(symbol, { t: Date.now(), d: dividends });
  return dividends;
}
